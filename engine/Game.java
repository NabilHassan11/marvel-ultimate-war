	...
	public static void loadChampions(String filePath) throws Exception{
		...
		while((champion = br.readLine()) != null){
			...
			for(int j = 0; j < availableAbilities.size();j++){
				String name = availableAbilities.get(j).getName();
				if(name.equals(arr[8]))
					locations[0] = j;
				else if(name.equals(arr[9]))
					locations[1] = j;
				else if(name.equals(arr[10]))
					locations[2] = j;
			} 
			...
			for(int i = 0; i < 3; i++)
				c.getAbilities().add(availableAbilities.get(locations[i]));
