...
public class Game {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					if (firstPlayer.getTeam().contains(getCurrentChampion()) && firstPlayer.getTeam().contains(target))
						throw new InvalidTargetException("You can't attack a team mate!");
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		ArrayList<Damageable> targets2 = new ArrayList<>();
	...
	private void getEnemy(Champion c,ArrayList<Champion> target){
		if(firstPlayer.getTeam().contains(c)){
			for(Champion tmp : secondPlayer.getTeam()){
				if(checkShield(tmp))
					target.add(tmp);
			}
		}
		else{
			for(Champion tmp : firstPlayer.getTeam()){
				if(checkShield(tmp))
					target.add(tmp);
			}
		}
